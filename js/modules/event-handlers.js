...
export const triggerAnomalyEvent = (intensity = 0.3) => {
    // Не запускаем аномалии в облегченном режиме
    if (window.performanceMode && window.performanceMode.isLightweightMode) {
...
export const setupEventHandlers = () => {
    ...
    cameraFeeds.forEach((feed) => {
        feed.addEventListener("click", () => {
            // В облегченном режиме полноэкранный режим работает по-другому
            if (window.performanceMode && window.performanceMode.isLightweightMode) {
            ...
            // Стандартное поведение для полного режима
            if (feed.classList.contains("fullscreen")) {
                feed.classList.remove("fullscreen");
                document.body.style.overflow = "auto";
            } else {
    ...
    toggleGridBtn.addEventListener("click", () => {
        // В облегченном режиме переключение сетки недоступно
        if (window.performanceMode && window.performanceMode.isLightweightMode) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    // Trigger random glitches (только в полном режиме)
                    setTimeout(() => {
                        if (!prefersReducedMotion && (!window.performanceMode || !window.performanceMode.isLightweightMode)) {
                            cameraFeeds.forEach((feed) => {
    ...
    triggerGlitchBtn.addEventListener("click", () => {
        // В облегченном режиме тест системы работает упрощенно
        if (window.performanceMode && window.performanceMode.isLightweightMode) {
            logEvent("ТЕСТ СИСТЕМЫ (ОБЛЕГЧЕННЫЙ РЕЖИМ)...");
            
            // Упрощенный визуальный эффект
    ...
    // Новые обработчики для провокации аномалии (только в полном режиме)
    if (!window.performanceMode || !window.performanceMode.isLightweightMode) {
